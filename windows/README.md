...
| 版本 | 日期 | 类型 | 说明 |
|------|------|------|------|
| **V2.8** | 2026-07-28 | 功能增强 | Web 控制台重构为响应式三栏工作台，新增三平台搜索结果“添加到播放列表”入口和 `/api/playlist/add`；新增 `/循环播放列表` 指令、网页列表循环按钮及 `/api/playlist/repeat`，歌曲播完后回到队尾并与单曲循环互斥；修复 `/播放列表` 的 KOOK `40011`；修复 B站 HTTPS 音频因 FFmpeg 不兼容 `-timeout` 参数而无输出、机器人进频道后秒退的问题，改用 `-rw_timeout` 并记录解码错误；更新 `/帮助` 与文档，相关功能同步至 Ubuntu |
...
- `tests/test_stability.py` 23 项加 `tests/test_watchdog.py` 12 项，共 35 项测试，覆盖播放并发与紧急恢复、单曲/列表循环互斥、歌曲完成后的循环入队、B站解码参数兼容性、播放列表安全文本输出、媒体进程身份校验、QQ 分页，以及看门狗宽限、网关失联与兼容降级、故障复位、组件恢复、重启预算、替代进程和路径确定性。
...
├── static/
│   ├── css/style.css                 # 全局样式
│   ├── css/dashboard.css             # 控制台三栏布局与响应式样式
│   └── js/
│       ├── main.js                   # 首页JS
...
| `/wy我的歌单` | 列出我的网易云歌单 | `/wy我的歌单` |
| `/qq 歌曲名` | 搜索并播放QQ音乐 | `/qq 晴天` |
| `/qqgd 歌单ID` | 导入QQ音乐歌单（支持 y.qq.com 链接） | `/qqgd 123456789` |
| `/qq我的歌单` | 列出我的QQ音乐歌单 | `/qq我的歌单` |
| `/暂停` | 暂停播放 | `/暂停` |
| `/继续` | 继续播放 | `/继续` |
| `/跳过` | 跳过当前歌曲 | `/跳过` |
| `/播放第N首` | 切到队列第N首歌 | `/播放第3首` |
| `/单曲循环` | 切换当前歌曲循环（开启时关闭列表循环） | `/单曲循环` |
| `/循环播放列表` | 切换列表循环，歌曲播完后回到队尾（开启时关闭单曲循环） | `/循环播放列表` |
| `/随机播放` | 切换播放队列的随机顺序 | `/随机播放` |
| `/停止` | 停止播放 | `/停止` |
| `/清空列表` | 清空当前播放队列 | `/清空列表` |
| `/脱离卡死` | 分阶段强制恢复播放会话 | `/脱离卡死` |
...
| `/版本信息` | 查看当前版本与最近版本历史 | `/版本信息` |
| `/cmd 命令` | 由 `CMD_ALLOWUSER` 白名单授权的 Windows CMD 命令 | `/cmd whoami` |
| `/帮助` | 显示所有可用指令及用法 | `/帮助` |
...
`CMD_ALLOWUSER` 为 `/cmd` 指令的独立用户白名单，**留空则全员无权限**（最安全策略）：

- `CMD_ALLOWUSER` 留空 → 所有用户均**无法**使用 `/cmd`
- `CMD_ALLOWUSER` 非空 → 仅名单内用户可执行，且需**同时**通过全局白名单
- 被拒绝时回复 `❌ 你没有权限执行CMD命令`
