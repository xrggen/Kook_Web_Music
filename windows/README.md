...
- FFmpeg/ffprobe 使用参数列表启动，不经过 `cmd.exe`；正常结束、异常和任务取消都会先终止并等待、排空 stdout/stderr 管道，再关闭播放线程事件循环，避免 Proactor 延迟析构时出现 `Event loop is closed`。
...
- `tests/test_stability.py` 24 项加 `tests/test_watchdog.py` 12 项，共 36 项测试，覆盖播放并发与紧急恢复、单曲/列表循环互斥、歌曲完成后的循环入队、B站解码参数兼容性、播放列表安全文本输出、线程事件循环关闭前的子进程管道回收、媒体进程身份校验、QQ 分页，以及看门狗宽限、网关失联与兼容降级、故障复位、组件恢复、重启预算、替代进程和路径确定性。
