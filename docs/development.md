...
注意：现有 `/api/terminal/command` 仍使用 `shell=True`，这是明确记录的遗留高风险边界，不应把这种模式复制到新代码。详见 [security.md](security.md)。
...
当前 connect 阶段验证 Session 和首次改密状态。
...
必须在事件处理器中再次执行 Role/Scope 校验。不要认为“连接已登录”就代表用户有权加入任意资源房间。
