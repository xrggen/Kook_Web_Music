...
- **安全连接与资源回收**：加入频道不再预先无条件离开；只有首次加入失败时才清理残留会话并重试。KOOK 请求复用带超时的 `aiohttp.ClientSession`；FFmpeg/ffprobe 在正常结束、异常和取消路径都会先终止并等待、排空 stdout/stderr 管道，再关闭播放线程事件循环，避免 Windows Proactor 延迟析构时出现 `Event loop is closed`。
...
- **验证**：`windows/tests/test_stability.py` 24 项加 `windows/tests/test_watchdog.py` 12 项，共 36 项测试，覆盖播放并发与停止竞态、单曲/列表循环互斥、歌曲完成后的循环入队、B站解码参数兼容性、播放列表安全文本输出、线程事件循环关闭前的子进程管道回收、紧急恢复、媒体进程身份校验、状态快照、QQ 分页，以及启动宽限、网关失联与兼容降级、连续故障复位、配置错误阻断、组件恢复、重启预算、替代进程和路径确定性。
...
- **Web控制台** — 三栏响应式工作台，醒目标示目标频道；搜索结果可随时添加到对应频道的播放列表
- **播放控制** — 播放 / 暂停 / 继续 / 跳过 / 停止 / 进度跳转 / 单曲循环 / 列表循环 / 随机播放
...
| | `/暂停` / `/继续` | 暂停 / 继续播放 |
| | `/跳过` / `/停止` | 跳过当前歌曲 / 停止播放 |
| | `/单曲循环` | 切换当前歌曲循环；开启时自动关闭列表循环 |
| | `/循环播放列表` | 切换列表循环；歌曲播完后回到队尾，开启时自动关闭单曲循环 |
| | `/随机播放` | 切换队列随机顺序 |
| | `/播放第N首` | 切到队列第N首歌 |
| | `/清空列表` | 清空播放队列 |
...
| | `/帮助` | 显示所有可用指令 |
| **系统** | `/ping` | 测试机器人连接 |
| | `/cmd 命令` | 由 `CMD_ALLOWUSER` 白名单授权的系统命令 |
...
| POST | `/api/playlist/add` | 添加歌曲到指定频道的播放列表（支持三平台） |
| POST | `/api/play` | 旧版单曲入队兼容入口 |
...
| POST | `/api/playlist/repeat` | 切换指定频道的列表循环模式 |
...
| B站支持 | 完整 | 完整（V2.7.2同步） |
| KOOK机器人命令 | 全部28个 | 全部28个 |
| 系统监控页面 | 无 | 有（monitor.html + Chart.js） |
| 终端实时输出 | 无 | 有 |
| 账号管理 | 完整（三平台扫码/Cookie） | 完整（三平台扫码/Cookie） |
| .env配置 | 通过config.py | 通过.env + config.py |
| 虚拟环境 | 无 | 在目标 Ubuntu 主机的 `Ubuntu/` 目录内创建 |

### 独立部署边界

- Windows 只能从 `windows/` 启动，使用该目录自己的网易云/QQ Node API 和 `ffmpeg/bin/ffmpeg.exe`、`ffprobe.exe`；`.env` 中的媒体工具路径应保持为相对路径。
- Ubuntu 只能从 `Ubuntu/` 启动，使用该目录自己的网易云/QQ Node API 与系统 `/usr/bin/ffmpeg`；不要复制或引用 Windows 的 `node_modules`、FFmpeg 或 Python 环境。
- `venv`、`node_modules` 均属于目标系统生成的依赖目录。跨机器部署时应分别在对应环境目录按 `requirements.txt`、`package.json` 重新安装，不能在两个版本间共享或建立链接。
